...
While there are many algorithms, probailisitic models are the most common:
...
## Latent Ditichler Allocation (LDA)

Assumptions:

- The order of words in the document does not matter (bag-of-words)
- The order of documents does not matter
- The numebr of topics is known and fixed

Topic assigment algorithm (using Gibbs sampling):
...
- For each token w in document d,
    ...
    - Probabilistic assignn token w a new topic based on:
        - The topics of other token in d
...
## Visual Explanation of LDA: Basics

We start with a vocabulary V:

![alt text](image-19.png)

Topics are treated as distributions of words in the vocabulary:

For example, we could consider the following topics:

![alt text](image-20.png)

In this example, we have the topics food (topic 1), emotions (topic 2), animals(topic 3).

## Visual Explanation of LDA: Topic Assigment

We can assign to documents by determining to which degree their words are drawn from the distribution of a given topic.

![alt text](image-21.png)

![alt text](image-22.png)

Of course, we don't know the topic distributions for a corpus in the first place and want to discover them. This is the problem that LDA solves.

We assume that each document can be generated by drawing words from the distributions of words that are the topics. Formally, we can model the space of documents as a Dirichlet distribution that is determined by the topics. Each document is a data point in this space.

![alt text](image-23.png)

The choice of topics defines this space for the input corpus. 
Changing the topics changes the distribution. LDA determines the latent distibution "hidden" in the corpus.

![alt text](image-24.png)

## Topic Models: A word of Caution

Topic models make two strong assumptions:

- The number of topics is known and fixed
- Bag-of-words

In comparison to contextual language models, this is relatively simplistic. Nevertheless, topic models can get extremely complex...

![alt text](image-25.png)

Don't simply apply topic models out-of-the-box just because they provided labeled clusters without first understanding the underlying assumptions!
